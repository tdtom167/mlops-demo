...
The databricks CLI bundles top level is defined by file `demo_mlops/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`demo_mlops/resources/model-workflow-resource.yml`)
 - batch inference workflow (`demo_mlops/resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`demo_mlops/resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`demo_mlops/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`demo_mlops/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `demo_mlops/databricks.yml` and `demo_mlops/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-demo_mlops-model     | /dev-demo_mlops-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-demo_mlops-model | /staging-demo_mlops-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-demo_mlops-model    | /prod-demo_mlops-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-demo_mlops-model    | /test-demo_mlops-experiment    |
...
2. Complete the `TODO` in `demo_mlops/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `demo_mlops/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `demo_mlops/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-demo_mlops-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-demo_mlops-batch-inference-job`
that runs the notebook under `demo_mlops/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-demo_mlops-batch-inference-job`.
...
`test-demo_mlops-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-demo_mlops-batch-inference-job`.
